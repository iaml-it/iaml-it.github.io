...
base_url: "https://iaml-it.github.io/"
...
```{r, out.width = "100%", fig.align="center", fig.cap = "The Data efficient image Transformer hard-label distillation procedure."}
...
```{r out.width = "100%", fig.align="center", fig.cap = "The Transformer in Transformer (TNT) architecture."}
...
```{r, out.width="100%", fig.align="center", fig.cap = "The Token-to-Token module architecture."}
knitr::include_graphics("t2t.png")
```
...
```{r out.width = "100%", fig.align="center", fig.cap = "The Pyramid Vision Transformer."}
knitr::include_graphics("pvt.png")
```
...
```{r out.width = "100%", fig.align="center", fig.cap = "The Swin Transformer shifted window partitioning procedure."}
knitr::include_graphics("swin2.png")
```
...
```{r out.width = "100%", fig.align="center", fig.cap = "Swin Transformer (left) and ViT (right). Attention is applied on patches (black) inside windows (red)."}
knitr::include_graphics("swin1.png")
```
...
[**LeViT: a Vision Transformer in ConvNet’s Clothing for Faster Inference**](https://arxiv.org/abs/2104.01136)[@levit] studied a hybrid architecture characterized by a longer convolutional stem and shrinking attention blocks that progressively diminish the spatial dimensions of the feature maps throughout the network. The authors also proposed the use of global average pooling instead of the class token, the injection of positional information through attention bias, and the addition of a [GELU activation](https://arxiv.org/abs/1606.08415)[@gelu] in the attention block.
...
```{r out.width = "100%", fig.align="center", fig.cap = "The Cross-Attention fusion layer for the large branch."}
knitr::include_graphics("crossvit.png")
```
...
```{r out.width = "100%", fig.align="center", fig.cap = "The (strided) Convolutional Projections."}
knitr::include_graphics("cvt.png")
```
...
```{r out.width = "100%", fig.align="center", fig.cap = "Global and local tokens in Multi-Scale Vision Longformers."}
knitr::include_graphics("vil.png")
```
...
```{r out.width = "100%", fig.align="center", fig.cap = "Blocked local Self-Attention as used in HaloNets."}
knitr::include_graphics("halonet.png")
```
...
```{r out.width = "100%", fig.align="center", fig.cap = "The So-ViT architecture."}
knitr::include_graphics("sovit.png")
```
...
We follow [**An Analysis of Deep Neural Network Models for Practical Applications**](https://openreview.net/forum?id=Bygq-H9eg)[@canziani] in graphing ImageNet top-1 accuracy vs. FLOPs and parameter count. A lot of models are displayed, so make sure to enable and disable the display of the various models (by clicking them in the legend) as to better explore the visualization.
...
As the recent [ResNet-RS paper](https://arxiv.org/abs/2103.07579)[@resnetrs] explains, *"in custom hardware architectures (e.g. TPUs and GPUs), operations are often bounded by memory access costs and have different levels of optimization on modern matrix multiplication units."* For these reasons, FLOPs are a particularly poor proxy for latency time.
...
In [**Group Equivariant Stand-Alone Self-Attention For Vision**](https://openreview.net/forum?id=JkfYjnOEo6M)[@gsa], the authors developed a self-attention formulation equivariant to arbitrary symmetry groups. The resulting models, called Group Self-Attention Networks (GSA-Nets), enjoy superior parameter efficiency, although not quite reaching that of comparable-size equivariant CNNs.

## Vision Transformers in Self-Supervised Learning

Building on top of previous studies ([iGPT](https://openai.com/blog/image-gpt/)[@igpt] and [masked patch prediction in the Vision Transformers paper](https://openreview.net/pdf?id=YicbFdNTTy#subsubsection.B.1.2)[@vit]), two papers have investigated self-supervised approaches using Vision Transformers.

[**An Empirical Study of Training Self-Supervised Vision Transformers**](https://arxiv.org/abs/2104.02057)[@random] identified an instability encountered during self-supervised training, causing performance degradation, particularly at large batch sizes. The authors propose a curious trick: a fixed, random patch projection, unlike normal ViTs where patch embeddings are learned just like the rest of the network.

```{r out.width = "100%", fig.align="center", fig.cap = "A comparison of training stability using learned and random patch embeddings."}
...
[**SiT: Self-supervised vIsion Transformer**](https://arxiv.org/abs/2104.03602)[@sit], studied a mix of three semi-supervised tasks (rotation prediction, image reconstruction, and a contrastive one) that, when combined, allow Vision Transformers to perform well on downstream tasks.

```{r out.width = "100%", fig.align="center", fig.cap = "The Self-supervised vIsion Transformer architecture."}
knitr::include_graphics("sit.png")
```
...
[**TransUNet: Transformers Make Strong Encoders for Medical Image Segmentation**](https://arxiv.org/abs/2102.04306)[@transunet] explored using a hybrid CNN-ViT encoder in an UNet-style architecture they call TransUNet. The authors reported favorable results in several segmentation tasks, especially when using a cascaded upsampler to obtain the final segmentation mask.

In [**Medical Transformer: Gated Axial-Attention for Medical Image Segmentation**](https://arxiv.org/abs/2102.10662)[@medt], the authors proposed the use of two branches, a shallower one, operating on the global context of the image, and a deeper one, operating locally on patches. The basic building block used for the model, which they named Medical Transformer (MedT), is (gated) axial self-attention[@axial]. The authors obtained improvements over baselines on several medical segmentation tasks.

```{r out.width = "100%", fig.align="center", fig.cap = "The Medical Transformer architecture."}
knitr::include_graphics("medt.png")
```

[**UNETR: Transformers for 3D Medical Image Segmentation**](https://arxiv.org/abs/2103.10504)[@unetr] adapted ViTs for 3D medical segmentation tasks. The authors showed that a simple adaptation was sufficient to improve over baselines on several 3D segmentation tasks. 

## Vision Transformers for Video Recognition

[**Video Transformer Network**](https://arxiv.org/abs/2102.00719)[@vtn] (VTN) proposed the use of a pre-trained 2D spatial backbone (the authors experiment with both CNN-based and ViT-based feature extractors) combined with a Transformer (in this case, a Longformer) operating on the extracted feature maps. The authors showed that this simple approach was competitive with baselines such as [SlowFast](https://arxiv.org/abs/1812.03982)[@slowfast].

[**Is Space-Time Attention All You Need for Video Understanding?**](https://arxiv.org/abs/2102.05095)[@timesformer] introduced TimeSformer, an adaption of ViTs to video. After exploring several possible attention variants, the authors proposed one called Divided Space-Time attention. In this approach, frames are first divided into patches and linearly embedded, then fed to a single transformer encoder. Every encoder layer has two attention blocks applied consecutively: the first one on patch embeddings at the same location but in different frames (temporal attention), the second on patch embeddings in the same frame (spatial attention).

The authors adopted ImageNet pretraining for spatial attention blocks and showed that the resulting models achieved state of the art, outperforming previous baselines on standard datasets such as [Kinetics-400](https://arxiv.org/abs/1705.06950)[@kinetics].

[**An Image is Worth 16x16 Words, What is a Video Worth?**](https://arxiv.org/abs/2103.13915)[@videoworth] proposed using a Vision Transformer to model the relationship between intra-frame patches, followed by a (temporal) Transformer encoder, modeling inter-frame relationships. It is conceptually similar to a VTN with a ViT-based spatial backbone.

The resulting models (called Spatio and Temporal Transformers, or STAMs) outperformed strong baselines such as [X3D](https://arxiv.org/abs/2004.04730)[@x3d] in the accuracy/FLOPs trade-off.

[**ViViT: A Video Vision Transformer**](https://arxiv.org/abs/2103.15691)[@vivit] discusses several approaches to adapt ViTs to video, and found the use of *tubelet embeddings*, linear projections of spatio-temporal tubes, to be most effective.

```{r out.width = "100%", fig.align="center", fig.cap = "Tubelet embeddings in ViViT."}
knitr::include_graphics("vivit.png")
```
...
Recently, [**Multiscale Vision Transformers**](https://arxiv.org/abs/2104.11227)[@mvit] proposed the adoption of a hierarchical structure by progressively increasing the feature dimension while decreasing the spatiotemporal one. This is achieved through the use of pooling, applied to keys and values in attention blocks.
...
At the time of writing, the only convolutional architecture still unsurpassed by transformers in efficiency is the recent [EfficientNetV2 family of models](https://arxiv.org/abs/2104.00298)[@efficientnetv2], obtained through extensive neural architecture search. No similarly extensive search has been performed for ViT-based models, but the small one reported in [**CvT: Introducing Convolutions to Vision Transformers**](https://arxiv.org/abs/2103.15808)[@cvt] achieved promising results (CvT-13-NAS).
...
* The amount of research coming out on a daily basis is significant. For this reason, we would like to highlight a few models for a more in-depth look, including the original [Vision Transformers](https://openreview.net/forum?id=YicbFdNTTy)[@vit], [DeiTs](https://arxiv.org/abs/2012.12877)[@deit], [TNTs](https://arxiv.org/abs/2103.00112)[@tnt], [ConViTs](https://arxiv.org/abs/2103.10697)[@convit], [Swin Transformers](https://arxiv.org/abs/2103.14030)[@swin], [CvTs](https://arxiv.org/abs/2103.15808)[@cvt], [CaiTs](https://arxiv.org/abs/2103.17239)[@cait], and [ViLs](https://arxiv.org/abs/2103.15358)[@vil].
...
The success of transformers in vision has had far-reaching consequences. We have discussed their influence on medical and video tasks, but their impact goes as far as audio (with [AST](https://arxiv.org/abs/2104.01778)[@ast], [Keyword Transformer](https://arxiv.org/abs/2104.00769)[@keyword]) and multimodal architectures (such as [ViLT](https://arxiv.org/abs/2102.03334)[@vilt] and [VATT](https://arxiv.org/abs/2104.11178)[@vatt]). Their success  has also inspired architectures with even fewer inductive biases, such as the [Perceiver](https://arxiv.org/abs/2103.03206)[@perceiver].
...
Thanks to [Simone Scardapane](https://twitter.com/s_scardapane), [Andrea Panizza](https://twitter.com/unsorsodicorda), [Amedeo Buonanno](https://twitter.com/linker81) and [Iacopo Poli](https://twitter.com/iacopo_poli) for help in proofreading and improving this work.